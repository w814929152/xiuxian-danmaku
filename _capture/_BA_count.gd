extends RefCounted
## BossArt 的一次性计数副本（draw 调用实测用），不参与正式构建
static var N := 0
static func tk(o: CanvasItem) -> CanvasItem:
	N += 1
	return o
## ---------------------------------------------------------------
...
static func _outline(ci: Node2D, pts: PackedVector2Array, col: Color, w: float) -> void:
	...
	ring.append(pts[0])
	tk(ci).draw_polyline(ring, col, w, true)
...
static func _dash_arc(ci: Node2D, rad: float, col: Color, w: float, segs: int,
	...
	for i in segs:
		...
		tk(ci).draw_arc(Vector2.ZERO, rad, a0, a1, 8, col, w, true)
...
static func _comb(ci: Node2D, org: Vector2, ux: Vector2, uy: Vector2,
	...
	while i >= 0:
		...
		i -= 1
	tk(ci).draw_colored_polygon(p, col)
...
static func _thruster(ci: Node2D, x: float, y: float, w: float, h: float,
	...
	tk(ci).draw_rect(Rect2(x, y, w, h), NEUTRAL)
	var fw := w * 0.72
	var fh := h * 0.60
	tk(ci).draw_rect(Rect2(x + w * 0.22, y + h * 0.20, fw, fh),
...
static func _sensor_row(ci: Node2D, c: Vector2, n: int, pitch: float,
	...
	tk(ci).draw_rect(Rect2(c.x - slot_w * 0.5 - 1.5, org.y - 1.5,
...
static func _portholes(ci: Node2D, c: Vector2, n: int, pitch: float,
	...
	tk(ci).draw_rect(Rect2(c.x - slot_w * 0.5 - 1.3, org.y - 1.3,
...
static func _view_slot(ci: Node2D, c: Vector2, w: float, h: float, dk: Color) -> void:
	tk(ci).draw_rect(Rect2(c.x - w * 0.5, c.y - h * 0.5, w, h), dk)
	tk(ci).draw_rect(Rect2(c.x - w * 0.5 + 1.5, c.y - h * 0.25, w - 3.0, h * 0.5), NEUTRAL)
...
static func _core_socket(ci: Node2D, r: float, c: int, pulse: float, ox: float) -> void:
	...
	tk(ci).draw_circle(Vector2(ox, 0.0), cr + r * 0.07, dk)
	tk(ci).draw_circle(Vector2(ox, 0.0), cr, k)
	tk(ci).draw_circle(Vector2(ox, 0.0), cr * 0.42, Color(1.0, 1.0, 1.0, 0.9))
...
static func _l1_rings(b: Node2D, _pulse: float) -> void:
	...
	if tell:
		...
		tk(b).draw_arc(Vector2.ZERO, r1, spin + gap * 0.5, spin + span + gap * 0.5,
			44, col, 5.0, true)
	...
	for i in 8:
		...
		var i0 := d * (r * 1.28)
		var i1 := d * (r * 1.38)
		tk(b).draw_colored_polygon(PackedVector2Array([i0 + pp, i1 + pp, i1 - pp,
			i0 - pp]), fcol)
	# 环上 3 枚符点（顺时针 1.1 rad/s）
	for i in 3:
		var a := spin + TAU * float(i) / 3.0
		tk(b).draw_circle(Vector2.RIGHT.rotated(a) * r1, 4.6, kcol)
	# 相②：第 2 圈 ×1.45，逆向自转
	if ph >= 2:
		var r2 := r * 1.45
		if tell:
			...
			tk(b).draw_arc(Vector2.ZERO, r2, -spin + gap * 0.5,
...
static func draw_core(b: Node2D, pulse: float) -> void:
	...
	tk(b).draw_colored_polygon(_bevel(SIL_CORE, r * 1.07, 0.08 * r), dk)
	# ④ MAIN 主体（硬底线：不透明 MAIN 实心块）+ 倒角
	tk(b).draw_colored_polygon(_bevel(SIL_CORE, r, 0.08 * r), m)
	# ⑤ NEUTRAL 结构件：内框六边
	...
	for i in 6:
		...
		tk(b).draw_line(Vector2.RIGHT.rotated(a) * (r * 0.62),
	...
	# ⑥ 熔炉辉环（GLOW，读作"炉子在烧"）
	tk(b).draw_arc(Vector2.ZERO, r * 0.40, 0.0, TAU, 28, Color(g.r, g.g, g.b, 0.55),
		4.0, true)
	# ⑧ CORE 要害核
...
static func _l2_wings(b: Node2D, _pulse: float) -> void:
	...
	for k in 2:
		...
		tk(b).draw_colored_polygon(memb, Color(g.r, g.g, g.b, 0.55))
		# 翼骨（承力梁）—— 三段折面的中轴
		tk(b).draw_polyline(PackedVector2Array([root, j1, j2, tip]), NEUTRAL, 4.0, true)
		# P2 翼段接缝 ×2（三段折面的段间缝，SEAM）—— 有接缝才读作「三段独立装甲板」
		var sew := maxf(1.5, 0.04 * r)
		tk(b).draw_line(j1 + perp * (w1 * 0.85), j1 - perp * (w1 * 0.62), SEAM, sew, true)
		tk(b).draw_line(j2 + perp * (w2 * 0.85), j2 - perp * (w2 * 0.62), SEAM, sew, true)
		# P3 翼根挂架（NEUTRAL 0.10r × 0.06r）—— 标志器官一律「挂在挂架上」
		tk(b).draw_rect(Rect2(root.x - 0.05 * r, root.y - 0.03 * r,
		...
		for i in 4:
			...
			tk(b).draw_colored_polygon(PackedVector2Array([p0 + pw, p1, p0 - pw]),
...
static func draw_wing(b: Node2D, pulse: float) -> void:
	...
	tk(b).draw_colored_polygon(_bevel(SIL_WING, r * 1.07, 0.06 * r), dk)
	tk(b).draw_colored_polygon(_bevel(SIL_WING, r, 0.06 * r), m)
	# ⑤ 结构线 ×2（寒霜四色结构线语言，§E.5 硬约束：器官不得覆盖它）
	tk(b).draw_line(Vector2(-0.34, -0.46) * r, Vector2(0.55, -0.10) * r, dk, 2.0, true)
	tk(b).draw_line(Vector2(-0.34, 0.46) * r, Vector2(0.55, 0.10) * r, dk, 2.0, true)
	...
	tk(b).draw_line(Vector2(-0.20, -0.33) * r, Vector2(0.55, -0.35) * r, cond, 3.0, true)
	tk(b).draw_line(Vector2(-0.20, 0.33) * r, Vector2(0.55, 0.35) * r, cond, 3.0, true)
...
static func _turret(ci: Node2D, r: float, y: float, g: Color) -> void:
	var bx := -0.646 * r
	tk(ci).draw_circle(Vector2(bx - r * 0.18, y), r * 0.055, Color(g.r, g.g, g.b, 0.95))
...
static func draw_battery(b: Node2D, pulse: float) -> void:
	...
	tk(b).draw_colored_polygon(_bevel(SIL_BATTERY, r * 1.06, 0.06 * r), dk)
	tk(b).draw_colored_polygon(sil, m)
	...
	for i in 3:
		...
		tk(b).draw_rect(Rect2(-0.514 * r, yy, 1.060 * r, 0.07 * r), NEUTRAL)
	...
	tk(b).draw_rect(Rect2(0.60 * r, -0.28 * r, 0.28 * r, 0.56 * r), NEUTRAL)
	tk(b).draw_rect(Rect2(0.86 * r, -0.14 * r, 0.16 * r, 0.28 * r),
...
static func _l4_tentacles(b: Node2D, _pulse: float) -> void:
	...
	if not stubs.is_empty():
		for k in stubs.size():
			tk(b).draw_polyline(stubs[k], dk, 5.0, true)
		return
...
static func _l4_tethers(b: Node2D, _pulse: float) -> void:
	...
	for i in cores.size():
		...
		tk(b).draw_line(tip, cp, Color(g.r, g.g, g.b, 0.60), 3.0, true)
		tk(b).draw_circle(cp, 5.0, Color(g.r, g.g, g.b, 0.85))
...
static func draw_maw(b: Node2D, pulse: float) -> void:
	...
	tk(b).draw_colored_polygon(_bevel(SIL_MAW, r * 1.06, 0.06 * r), dk)
	tk(b).draw_colored_polygon(sil, m)
	# ⑤ NEUTRAL 结构件 30%：上下两条纵向装甲带
	tk(b).draw_rect(Rect2(-0.30 * r, -0.86 * r, 1.10 * r, 0.09 * r), NEUTRAL)
	tk(b).draw_rect(Rect2(-0.30 * r, 0.77 * r, 1.10 * r, 0.09 * r), NEUTRAL)
	...
	if exposed:
		...
		tk(b).draw_circle(Vector2(mx, 0.0), r * 0.42, dk)
	else:
		tk(b).draw_circle(Vector2(mx, 0.0), r * 0.42, Color(0.04, 0.03, 0.08, 0.95))
		tk(b).draw_arc(Vector2(mx, 0.0), r * 0.42, PI * 0.55, PI * 1.45, 20,
...
static func _l5_echoes(b: Node2D, _pulse: float) -> void:
	...
	for i in n:
		...
		for j in base.size():
			...
			pts[j] = q + off
		tk(b).draw_colored_polygon(pts, Color(STEEL.r, STEEL.g, STEEL.b, alpha))
		var ring := PackedVector2Array(pts)
		ring.append(pts[0])
		tk(b).draw_polyline(ring, Color(STEEL_GLOW.r, STEEL_GLOW.g, STEEL_GLOW.b,
			alpha * 0.80), 2.0, true)
...
static func draw_throne(b: Node2D, pulse: float) -> void:
	...
	tk(b).draw_colored_polygon(_bevel(SIL_THRONE, r * 1.06, 0.06 * r), dk)
	tk(b).draw_colored_polygon(sil, m)
	# ⑤ NEUTRAL 结构线：中轴脊 + 上下弦（保住「颜色 + 形状」双通道的形状侧）
	tk(b).draw_line(Vector2(-0.90 * r, 0.0), Vector2(0.72 * r, 0.0), NEUTRAL_DK, 3.0, true)
	tk(b).draw_line(Vector2(-0.40 * r, -0.22 * r), Vector2(0.52 * r, -0.12 * r),
		NEUTRAL, 2.0, true)
	tk(b).draw_line(Vector2(-0.40 * r, 0.22 * r), Vector2(0.52 * r, 0.12 * r),
		NEUTRAL, 2.0, true)
	...
	for i in 3:
		...
		tk(b).draw_polyline(PackedVector2Array([Vector2(xx, -0.30 * r),
