...
## 中性钢色：机械结构件（推进器喷嘴壳 / 挂架 / 接缝底）专用。
## 刻意**不绑定属性**：属性色通道留给本体与能量部件，结构件一律灰，
## 免得四色甲各自的「机械感」被染成四套不同颜色而与 BossArt 的 NEUTRAL 对不上。
...
const ARMOR_TAG := ["充能 · 贯穿激光", "闪避 0.5 秒", "护盾 20", "引力束 · 过热"]
# 战甲库里的详述（一行一段，比卡面文案更细）
const ARMOR_LORE := [
	"电浆剑甲以等离子熔核驱动，装具与电浆弹头共振。\n出刃时剑控自生双影，一次递出两排光刃。\n吞下的电浆弹尽入熔核 —— 满百即化作一道贯穿激光。",
