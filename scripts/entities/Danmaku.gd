...
extends Area2D
## 敌方能量弹（电浆 / 寒霜 / 光子 / 引力）
## 四色**同形**白模弹丸：「灵纹能量珠」—— 白炽内核 + 灵纹刻线 + 暗缘收边 + 染色光晕，
## 由 tools/build_danmaku.py 程序化烘焙（幂等可重跑，断言锁定剪影尺寸）。
## 运行期 modulate 染成属性色，形状统一、只靠颜色区分属性 ——
## 四色异形（形状各异）在弹幕高峰时会让画面过于杂乱。
##
## Phase 4：
...
## 白模按「基准半径 9」烘焙（24px 画布）。其余半径靠 scale 缩放；换图必须同步这里。
