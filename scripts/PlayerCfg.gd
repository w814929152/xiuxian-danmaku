...
# ============================ 电浆剑甲 · 贯穿激光 ============================
## 每吞下一枚电浆弹攒的能量。**只在穿着电浆剑甲时攒**（见 Player.take_hit）——
## 与光子护盾的充能同制式：判定排在力场罩早退之前，力场罩期间吞弹照攒。
const CHARGE_GAIN := 20
## 能量上限：攒满即**自动**打出一道贯穿激光，随后清零重攒。
## 20 × 5 = 100 —— 第五颗电浆弹触发，一局里够读出来「快满了」。
const CHARGE_MAX := 100
## 贯穿激光的伤害倍率（相对**单发光刃**，即 SWORD_DMG_RED × 本值）。
## 走 Player.lance_damage() 一处派生 —— 增幅核心的加成因此自然生效，
## 别在别处再乘一遍（改了公式要全局搜的教训）。
const LANCE_MUL := 3.0
## 光柱半高：判定高度 = 本值 × 2。与引力束 BEAM_HALF_H=9 相比刻意更粗 ——
## 它是「攒五发才来一次」的重击，窄一点玩家会觉得打出去没手感。
const LANCE_HALF_H := 16.0
## 收束视觉时长。伤害在生成后的**第一个物理帧**一次性结算完，
## 这个数只决定光柱在屏幕上留多久 —— 太长会挡住弹幕，太短看不见。
## ⚠️ 0.30 秒实测「一闪就没了」，玩家根本没意识到自己打出过东西（攒五发才来一次，
##    给了反馈却看不见等于没给）。0.55 秒是读了截帧定的：约 17~33 帧，
##    贯穿一排的因果看得清，又不至于长时间糊住右半屏的弹幕。
const LANCE_LIFE := 0.55
